...
#    ⚠️ VM 是 amd64；在 Apple Silicon (arm64) Mac 上 build 必須指定
#    --platform linux/amd64，否則容器會 exec format error 無法啟動。
cd frontend
docker build --platform linux/amd64 --build-arg NEXT_PUBLIC_R2_URL=https://img.paulfun.net -t paulfun-frontend:prod .

cd ../backend-go
docker build --platform linux/amd64 -t paulfun-go-server:prod .
...
#    VM 上的 docker 需要 sudo；compose 指令為 docker-compose（連字號）。
#    只重建單一服務可加 --no-deps --force-recreate frontend（或 go-server）。
gcloud compute ssh paul-ubuntu --zone=asia-east1-a --project=paul-test-174403 --command="\
  cd ~/paulfun-blogger && \
  sudo docker load < frontend.tar.gz && \
  sudo docker load < go-server.tar.gz && \
